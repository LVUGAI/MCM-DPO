...
def concate_pad_many(tensor_list, padding_value):
    result_list = []
...
def mcmdpo_get_batch_logps(logits: torch.FloatTensor,
    ...
    if average_log_prob:
        return (per_policy_token_logps * loss_mask).sum(-1) / loss_mask.sum(-1), \
                (per_reference_token_logps * loss_mask).sum(-1) / loss_mask.sum(-1), \
                per_policy_token_logps, per_reference_token_logps
    else:
        return (per_policy_token_logps * loss_mask).sum(-1), \
...
class LLAVADPOTrainer(LLaVATrainer):
    ...
    @staticmethod
    def dpo_loss(policy_chosen_logp: torch.FloatTensor,
    ...
    def mcmdpo_loss(self, 
                    policy_cw_mw_yw_logp, policy_cw_mw_yl_logp,
                    policy_cw_ml_yw_logp, policy_cw_ml_yl_logp, 
                    policy_cl_mw_yw_logp, policy_cl_mw_yl_logp, 
                    policy_cl_ml_yw_logp, policy_cl_ml_yl_logp,      
                    ref_cw_mw_yw_logp, ref_cw_mw_yl_logp, 
                    ref_cw_ml_yw_logp, ref_cw_ml_yl_logp, 
                    ref_cl_mw_yw_logp, ref_cl_mw_yl_logp, 
                    ref_cl_ml_yw_logp, ref_cl_ml_yl_logp, 

                    beta: float=0.1, gama:float=0.3
                    ) -> Tuple[
        torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the mcmDPO loss for a batch of policy and reference model log probabilities."""

        pi_logratios = self.args.para_r3 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cw_mw_yl_logp)
        ref_logratios = self.args.para_r3 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cw_mw_yl_logp)
        logits = pi_logratios - ref_logratios
        if self.args.use_cross_modal_loss:
            logits += self.args.para_r2 / self.args.para_norm * (policy_cw_mw_yw_logp - ref_cw_mw_yw_logp)
            logits -= self.args.para_r2 / self.args.para_norm * (policy_cw_ml_yw_logp - ref_cw_ml_yw_logp)        
            if self.args.use_cl_reward_num > 2:
                logits += self.args.para_r1 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cl_mw_yw_logp)
                logits -= self.args.para_r1 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cl_mw_yw_logp)
                if self.args.use_cl_reward_num > 3:
                    logits += self.args.para_r4 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cl_mw_yl_logp)
                    logits -= self.args.para_r4 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cl_mw_yl_logp)
                    if self.args.use_cl_reward_num > 4:
                        logits += self.args.para_r5 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cw_ml_yl_logp)
                        logits -= self.args.para_r5 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cw_ml_yl_logp)
                        if self.args.use_cl_reward_num > 5:
                            logits += self.args.para_r6 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cl_ml_yw_logp)
                            logits -= self.args.para_r6 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cl_ml_yw_logp)
                            if self.args.use_cl_reward_num > 6:
                                logits += self.args.para_r7 / self.args.para_norm * (policy_cw_mw_yw_logp - policy_cl_ml_yl_logp)
                                logits -= self.args.para_r7 / self.args.para_norm * (ref_cw_mw_yw_logp - ref_cl_ml_yl_logp)

        chosen_values = policy_cw_mw_yw_logp - ref_cw_mw_yw_logp
        rejected_values = policy_cw_mw_yl_logp - ref_cw_mw_yl_logp
    ...
    def compute_loss(self, model: Module, inputs: dict, return_outputs=False):
        ...
        win_cl_input_ids = data_dict.pop('win_cl_input_ids')
        rej_cl_input_ids = data_dict.pop('rej_cl_input_ids')
        ...
        win_cl_size = win_cl_input_ids.shape[0]
        rej_cl_size = rej_cl_input_ids.shape[0]
        ...
        concatenated_input_ids_8 = data_dict.pop('concatenated_input_ids_8')
        concatenated_labels_8 = data_dict.pop('concatenated_labels_8')
        concatenated_attention_mask_8 = data_dict.pop('concatenated_attention_mask_8')
        ...
        if self.args.use_cross_modal_loss:
            output, new_labels = model(
                input_ids=concatenated_input_ids_8,
                labels=concatenated_labels_8,
                attention_mask=concatenated_attention_mask_8,
                images=torch.cat([
                    images, images, diffusion_image, diffusion_image, # c_w
                    images, images, diffusion_image, diffusion_image, # c_l
                    ], dim=0),
        ...
        policy_logps, ref_logps, per_policy_token_logps, per_reference_token_logps, \
                = mcmdpo_get_batch_logps(output.logits, ref_output.logits,
        ...
        if self.args.use_cross_modal_loss:
            ...
            policy_cl_ml_yw_logp, policy_cl_ml_yl_logp, \
                = policy_logps.split([
                win_size, rej_size, win_size, rej_size, 
                win_cl_size, rej_cl_size, win_cl_size, rej_cl_size,
                ])
            ref_cw_mw_yw_logp, ref_cw_mw_yl_logp, \
            ref_cw_ml_yw_logp, ref_cw_ml_yl_logp, \
            ref_cl_mw_yw_logp, ref_cl_mw_yl_logp, \
            ...
            losses, chosen_rewards, rejected_rewards = self.mcmdpo_loss(
            policy_cw_mw_yw_logp, policy_cw_mw_yl_logp, 
            policy_cw_ml_yw_logp, policy_cw_ml_yl_logp, 
            policy_cl_mw_yw_logp, policy_cl_mw_yl_logp, 
            policy_cl_ml_yw_logp, policy_cl_ml_yl_logp,      
            ...
            policy_chosen_logp, policy_rejected_logp = policy_logps.split([win_size, rej_size])
