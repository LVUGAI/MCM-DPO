...
from llava.train.llava_trainer import LLaVATrainer, LLAVADPOTrainer, get_batch_logps, concate_pad, concate_pad_three, concate_pad_many
...
class DPODataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        random.seed(i)
        while True:
            random_idx = random.choice(range(len(self.list_data_dict)))
            if random_idx != i:
                break

        random_sample: dict = self.list_data_dict[random_idx]

        text = json.loads(sample['text'])
        random_text = json.loads(random_sample['text'])
        question = {'from': 'human', 'value': f"<image>\n{text['question']}"}
        random_question = {'from': 'human', 'value': f"<image>\n{random_text['question']}"}
        chosen = {'from': 'gpt', 'value': text['chosen']}
        ...
        cl_win_conv = copy.deepcopy([source['random_question'], source["chosen"]])
        cl_rej_conv = copy.deepcopy([source['random_question'], source["rejected"]])
        ...
        if 'image' in source:
            ...
            win_conv = preprocess_multimodal([win_conv], self.data_args)[0]
            cl_win_conv = preprocess_multimodal([cl_win_conv], self.data_args)[0]
            rej_conv = preprocess_multimodal([rej_conv], self.data_args)[0]
            cl_rej_conv = preprocess_multimodal([cl_rej_conv], self.data_args)[0]
        rej_data_dict = preprocess([rej_conv], self.tokenizer, has_image='image' in source)
        cl_rej_data_dict = preprocess([cl_rej_conv], self.tokenizer, has_image='image' in source)
        rej_data_dict = dict(
            input_ids=rej_data_dict["input_ids"][0], 
            labels=rej_data_dict["labels"][0],
            cl_input_ids=cl_rej_data_dict["input_ids"][0], 
            cl_labels=cl_rej_data_dict["labels"][0],
        )

        win_data_dict = preprocess([win_conv], self.tokenizer, has_image='image' in source)
        cl_win_data_dict = preprocess([cl_win_conv], self.tokenizer, has_image='image' in source)
        win_data_dict = dict(
            input_ids=win_data_dict["input_ids"][0],
            labels=win_data_dict["labels"][0],
            cl_input_ids=cl_win_data_dict["input_ids"][0],
            cl_labels=cl_win_data_dict["labels"][0],
            idx=sample['idx'],
        )
...
class DataCollatorForDPODataset(object):
    ...
    def SFT_collator_fn(self, instances, pad_token_id):
        input_ids, labels, cl_input_ids, cl_labels = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "labels", "cl_input_ids", "cl_labels"))
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids,
            batch_first=True,
        ...
        labels = torch.nn.utils.rnn.pad_sequence(
            ...
            batch_first=True,
            padding_value=IGNORE_INDEX)
        cl_input_ids = torch.nn.utils.rnn.pad_sequence(
            cl_input_ids,
            batch_first=True,
            padding_value=pad_token_id)
        cl_labels = torch.nn.utils.rnn.pad_sequence(
            cl_labels,
            batch_first=True,
            padding_value=IGNORE_INDEX)
        batch = dict(
            ...
            cl_input_ids=cl_input_ids,
            cl_labels=cl_labels,
            # cl_attention_mask=cl_input_ids.ne(pad_token_id),
        )
    ...
    def preference_collator_fn(self, instances, pad_token_id):
        ...
        concatenated_input_ids_3 = concate_pad_three(win_batch['input_ids'], rej_batch['input_ids'], win_batch['input_ids'], pad_token_id)
        ...
        concatenated_input_ids_8 = concate_pad_many(
            [
                win_batch['input_ids'], rej_batch['input_ids'], 
                win_batch['input_ids'], rej_batch['input_ids'], 
                win_batch['cl_input_ids'], rej_batch['cl_input_ids'], 
                win_batch['cl_input_ids'], rej_batch['cl_input_ids']
            ], 
            pad_token_id)
        concatenated_labels_8 = concate_pad_many(
            [
                win_batch['labels'], rej_batch['labels'], 
                win_batch['labels'], rej_batch['labels'], 
                win_batch['cl_labels'], rej_batch['cl_labels'], 
                win_batch['cl_labels'], rej_batch['cl_labels']
            ], 
            -100)
        concatenated_attention_mask_8 = concatenated_input_ids_8.ne(pad_token_id)
        ...
        batch = dict(
            ...
            concatenated_input_ids_8=concatenated_input_ids_8,
            concatenated_labels_8=concatenated_labels_8,
            concatenated_attention_mask_8=concatenated_attention_mask_8,
            win_input_ids=win_batch['input_ids'],
            rej_input_ids=rej_batch['input_ids'],
            win_cl_input_ids=win_batch['cl_input_ids'],
            rej_cl_input_ids=rej_batch['cl_input_ids'],
    ...
    if training_args.task == 'DPO':
        if data_args.offline_ref_logits:
            ...
            if not os.path.exists(data_args.offline_ref_logits):
                ...
                #     get_model_name_from_path(model_args.model_name_or_path)
                # )
                ref_model = get_ref_model(model_args.model_name_or_path, training_args, model_args)
                # ref_model = copy.deepcopy(model).to(device=training_args.device, dtype=torch.bfloat16)
